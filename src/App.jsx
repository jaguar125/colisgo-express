...
.onboard{
  min-height:100vh; min-height:100dvh;
  background:
    radial-gradient(60% 45% at 85% 0%, rgba(245,166,35,0.22) 0%, transparent 60%),
    radial-gradient(70% 55% at 8% 8%, rgba(125,217,206,0.16) 0%, transparent 60%),
    radial-gradient(120% 90% at 15% 0%, #1B2C66 0%, var(--ink) 55%, #081026 100%);
  color:var(--paper-2);
  display:flex;
  flex-direction:column;
...
.onboard::after{
  content:'';
  position:absolute; top:-120px; right:-90px; width:280px; height:280px; border-radius:50%;
  background:radial-gradient(circle, rgba(245,166,35,0.35) 0%, transparent 70%);
  filter:blur(10px); pointer-events:none; animation:orbFloat 7s ease-in-out infinite;
}
@keyframes orbFloat{
...
.onboard-mark{
  ...
  background:linear-gradient(150deg, var(--amber) 0%, #E08A0F 100%);
  display:flex;align-items:center;justify-content:center;
  transform:rotate(-8deg);
  box-shadow:0 14px 34px rgba(245,166,35,0.45), 0 0 0 6px rgba(245,166,35,0.1);
  flex-shrink:0;
  animation:markPulse 3.2s ease-in-out infinite;
}
@keyframes markPulse{
  0%,100%{ box-shadow:0 14px 34px rgba(245,166,35,0.45), 0 0 0 6px rgba(245,166,35,0.1); }
  50%{ box-shadow:0 14px 34px rgba(245,166,35,0.6), 0 0 0 10px rgba(245,166,35,0.16); }
}
.onboard-brand-row{
  display:flex;
  align-items:center;
...
.onboard h1{
  ...
  background:linear-gradient(100deg, #fff 30%, #FFD98F 65%, var(--amber) 100%);
  -webkit-background-clip:text; background-clip:text; color:transparent;
}
.onboard p.tag{ color:#C7CFEC; font-size:1.02rem; line-height:1.55; max-width:36ch; margin-top:20px; position:relative; z-index:1; }
.onboard-trust-row{
  display:flex; gap:8px; margin-top:20px; flex-wrap:wrap; position:relative; z-index:1;
}
.onboard-trust-pill{
  ...
  font-size:0.72rem; font-weight:600; color:#EDEFFA; backdrop-filter:blur(6px);
}
.onboard-trust-pill svg{ color:var(--amber); }
.route-strip{
  display:flex; align-items:center; gap:10px;
  margin-top:30px; color:var(--paper-2); opacity:0.9; position:relative; z-index:1;
}
.route-strip .code{ font-family:'JetBrains Mono',monospace; font-weight:600; letter-spacing:0.08em; font-size:0.85rem; }
.route-strip .line{ flex:1; height:1px; background:linear-gradient(90deg,transparent,#7D89B8,transparent); position:relative; overflow:hidden; }
.route-strip .line::after{
...
.onboard-form label{ font-size:0.8rem; color:#C7CFEC; display:block; margin-bottom:8px; font-weight:600; }
.onboard-form input{
  width:100%; padding:15px 16px; border-radius:13px; border:1.5px solid rgba(255,255,255,0.14);
  background:rgba(10,17,45,0.55); color:#fff; font-size:1rem; font-family:'Inter',sans-serif;
  transition:border-color .15s ease, background .15s ease;
}
.onboard-form input:focus{ outline:none; border-color:var(--amber); background:rgba(10,17,45,0.8); }
.onboard-form input::placeholder{ color:#7681AC; }
.btn-amber{
  margin-top:20px; width:100%; padding:17px; border-radius:13px; border:none;
  background:linear-gradient(135deg, #FFC24D 0%, var(--amber) 55%, #D9880F 100%);
  color:#241300; font-weight:800; font-size:1.02rem;
...
.btn-amber:hover{ box-shadow:0 12px 34px rgba(245,166,35,0.55); transform:translateY(-1px); }
.onboard-foot{ font-size:0.72rem; color:#7681AC; margin-top:26px; line-height:1.5; position:relative; z-index:1; }
.back-to-landing{
...
.onboard-divider{
  display:flex; align-items:center; gap:12px; margin:18px 0; color:#7681AC; font-size:0.75rem;
}
.onboard-divider::before, .onboard-divider::after{ content:''; flex:1; height:1px; background:#2C3C74; }
.btn-google{
  width:100%; padding:14px; border-radius:13px; border:1.5px solid rgba(255,255,255,0.16);
  background:rgba(17,27,64,0.6); color:#EDEFFA; font-weight:600; font-size:0.92rem;
...
const CAROUSEL_IMAGES = [
  {
    src: 'https://images.unsplash.com/photo-1702134731681-97d4f829b4a6?w=500&q=70&auto=format&fit=crop',
  ...
  {
    src: 'https://images.unsplash.com/photo-1583193123517-09ef9447412c?w=500&q=70&auto=format&fit=crop',
      ...
      <div className="landing-shell">
        <div className="landing-hero">
          <div className="hero-bg">
            <img
              src="https://images.unsplash.com/photo-1702134731681-97d4f829b4a6?w=900&q=75&auto=format&fit=crop"
...
function ChatWidget() {
  ...
  useEffect(() => {
    if (open && identity) {
      ...
      pollRef.current = setInterval(() => loadThread(identity.token), 3000);
...
export default function App() {
  ...
  const [expandedAdminShipments, setExpandedAdminShipments] = useState({});
  const [expandedAdminUsers, setExpandedAdminUsers] = useState({});
  const [expandedAdminReviews, setExpandedAdminReviews] = useState({});
  const [expandedAdminWithdrawals, setExpandedAdminWithdrawals] = useState({});
  const [expandedMyTrips, setExpandedMyTrips] = useState({}); // Mes trajets — published trips
  const [expandedIncoming, setExpandedIncoming] = useState({}); // Mes trajets — incoming requests
  const [expandedSupportThreads, setExpandedSupportThreads] = useState({}); // admin — messages de contact
  ...
  useEffect(() => {
    ...
    notifPollRef.current = setInterval(poll, 3000);
    ...
    withBusy(async () => {
      ...
      try {
        ...
        await sbRest(`profiles?id=eq.${profile.id}`, {
          ...
          token,
          body: {
            nom_complet: `${data.prenom.trim()} ${data.nom.trim()}`.trim(),
  ...
  const confirmDelivery = (shipment) =>
    withBusy(async () => {
      ...
      try {
        // Upsert (not a plain insert): if a proof already exists for this
        // matching — e.g. a previous attempt got this far but the next step
        // below failed for some other reason — this updates it in place
        // instead of hitting the "one proof per matching" unique constraint
        // and getting permanently stuck.
        await sbRest('preuves_livraison?on_conflict=matching_id', {
          method: 'POST',
          token,
        ...
        <div className="cge-shell">
          <div className="onboard">
            <div>
              ...
              <div className="onboard-trust-row">
                <span className="onboard-trust-pill">
  ...
  // Only the most recent matching per colis is shown — once a new offer is
  // submitted after a refusal, the old refused proposal for that same
  // package disappears instead of lingering alongside the new one.
  const myShipmentsAsExpediteur = (() => {
    const mine = shipments.filter((s) =>
      s.expediteurEmail ? s.expediteurEmail === profile.email : s.expediteurName === profile.name
    );
    const latestByColis = {};
    mine.forEach((s) => {
      const key = s.colisId || s.id;
      if (!latestByColis[key] || s.createdAt > latestByColis[key].createdAt) {
        latestByColis[key] = s;
      }
    });
    return Object.values(latestByColis).sort((a, b) => b.createdAt - a.createdAt);
  })();
          ...
          {tab === 'mestrajets' && (
            <>
              ...
              {myTrips.length > 0 && (
                <>
                  ...
                  {myTrips.map((t) => {
                    ...
                    const myTripExpanded = expandedMyTrips[t.id] === true;
                    return (
                        ...
                        <div className="ship-top">
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <button
                              ...
                              className="icon-btn"
                              style={{ width: 26, height: 26 }}
                              onClick={() => setExpandedMyTrips((prev) => ({ ...prev, [t.id]: !myTripExpanded }))}
                              aria-label={myTripExpanded ? 'Réduire' : 'Déplier'}
                            >
                              {myTripExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                            </button>
                          </div>
                        </div>
                        {myTripExpanded && (
              ...
              {incomingShipments.map((s) => {
                const incomingExpanded = expandedIncoming[s.id] === true;
                return (
                <div className="ship-card" key={s.id}>
                  <div className="ship-top">
                    <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button
                          ...
                          className="icon-btn"
                          style={{ width: 26, height: 26 }}
                          onClick={() => setExpandedIncoming((prev) => ({ ...prev, [s.id]: !incomingExpanded }))}
                          aria-label={incomingExpanded ? 'Réduire' : 'Déplier'}
                        >
                          {incomingExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                        </button>
                      </div>
                      {s.paymentSkipped && (
                        <span className="badge" style={{ background: '#F0EEE5', color: 'var(--slate)' }}>
                        ...
                        <div className="tx-left">
                          ...
                          <div>
                            ...
                            <div className="tx-sub">
                              ...
                              ·{' '}
                              {w.statut === 'reussi' ? (
                                <b style={{ color: '#0F7A3D' }}>Traité</b>
                              ) : w.statut === 'echoue' ? (
                                <b style={{ color: 'var(--coral)' }}>Échoué</b>
                              ) : (
          ...
          {tab === 'admin' && isAdmin && (
            <>
              ...
              {adminSection === 'shipments' && (
                <>
                  ...
                  {shipmentsSearch.items.map((s) => {
                    const shipExpanded = expandedAdminShipments[s.id] === true;
                    return (
                    <div className="ship-card" key={s.id}>
                      <div className="ship-top">
                        <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button
                            ...
                            className="icon-btn"
                            style={{ width: 26, height: 26 }}
                            onClick={() => setExpandedAdminShipments((prev) => ({ ...prev, [s.id]: !shipExpanded }))}
                            aria-label={shipExpanded ? 'Réduire' : 'Déplier'}
                          >
                            {shipExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                          </button>
                        </div>
                      </div>
                      {shipExpanded && (
              ...
              {adminSection === 'users' && (
                <>
                  ...
                  {usersSearch.items.map((u) => {
                    const userExpanded = expandedAdminUsers[u.id] === true;
                    return (
                      ...
                      <div className="ship-top">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button
                            ...
                            className="icon-btn"
                            style={{ width: 26, height: 26 }}
                            onClick={() => setExpandedAdminUsers((prev) => ({ ...prev, [u.id]: !userExpanded }))}
                            aria-label={userExpanded ? 'Réduire' : 'Déplier'}
                          >
              ...
              {adminSection === 'kyc' && (
                <>
                  ...
                  {kycSearch.items.map((r) => {
                    const kycExpanded = expandedKyc[r.id] === true;
                    return (
                    <div className="ship-card" key={r.id}>
                      ...
                      {kycExpanded && (
                      <>
                      <div className="ship-grid" style={{ gridTemplateColumns: '1fr', marginBottom: 2 }}>
                        <div>
                          <div className="k">Prénom et nom (formulaire)</div>
                          <div className="v" style={{ fontSize: '0.95rem' }}>{r.name}</div>
                        </div>
                      </div>
                      <div className="ship-grid">
                        <div>
              ...
              {tripsSearch.items.map((t) => {
                const expired = isTripExpired(t);
                const adminTripExpanded = expandedAdminTrips[t.id] === true;
                return (
                  <div className="ship-card" key={t.id}>
                    <div className="ship-top">
                      <div>
              ...
              {reviewsSearch.items.map((r) => {
                const reviewExpanded = expandedAdminReviews[r.id] === true;
                return (
                <div className="ship-card" key={r.id}>
                  <div className="ship-top">
                    <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <button
                        ...
                        className="icon-btn"
                        style={{ width: 26, height: 26 }}
                        onClick={() => setExpandedAdminReviews((prev) => ({ ...prev, [r.id]: !reviewExpanded }))}
                        aria-label={reviewExpanded ? 'Réduire' : 'Déplier'}
                      >
                        {reviewExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                      </button>
                    </div>
                  </div>
                  {reviewExpanded && (
              ...
              {adminSection === 'messages' && (
                <>
                  ...
                  {supportThreads.map((t) => {
                    const threadExpanded = expandedSupportThreads[t.token] === true;
                    return (
                      ...
                      <div className="ship-top">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button
                            ...
                            style={{ width: 26, height: 26 }}
                            onClick={() =>
                              setExpandedSupportThreads((prev) => ({ ...prev, [t.token]: !threadExpanded }))
                            }
                            aria-label={threadExpanded ? 'Réduire' : 'Déplier'}
                          >
                            {threadExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                          </button>
                        </div>
                      </div>
              ...
              {adminSection === 'retraits' && (
                <>
                  ...
                  {withdrawalsSearch.items.map((w) => {
                    const wExpanded = expandedAdminWithdrawals[w.id] === true;
                    return (
                      ...
                      <div className="ship-top">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button
                            ...
                            className="icon-btn"
                            style={{ width: 26, height: 26 }}
                            onClick={() => setExpandedAdminWithdrawals((prev) => ({ ...prev, [w.id]: !wExpanded }))}
                            aria-label={wExpanded ? 'Réduire' : 'Déplier'}
                          >
                            {wExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                          </button>
                        </div>
                      </div>
                      {wExpanded && (
