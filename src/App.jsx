...
export default function App() {
  ...
  const resubmitOffer = (shipment, newPriceRaw) =>
    withBusy(async () => {
      ...
      const minPrice = shipment.declaredValue > 0 ? Math.round(shipment.declaredValue * 0.12 * 100) / 100 : 0;
      if (newPrice < minPrice) {
        notify(`Le tarif doit être d'au moins ${fcfa(minPrice)} (12 % de la valeur déclarée du colis)`, 'error');
        return;
      }
      try {
        await sbRest('matchings', {
...
function ShipmentFormSheet({ trip, onClose, onSubmit }) {
  ...
  const minPrice = declaredValue && parseFloat(declaredValue) > 0 ? Math.round(parseFloat(declaredValue) * 0.12 * 100) / 100 : 0;
  const priceValid =
    proposedPrice !== '' &&
    ...
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={'field' + (touched.proposedPrice && !priceValid ? ' has-error' : '')}>
          ...
          {minPrice > 0 && (
            <p className="helper">
              Minimum requis : <b>{fcfa(minPrice)}</b> (au moins 12&nbsp;% de la valeur déclarée du
              colis).
            </p>
          )}
          <FieldError show={touched.proposedPrice && !priceValid}>
            {proposedPrice !== '' && parseFloat(proposedPrice) < minPrice
              ? `Le tarif doit être d'au moins ${fcfa(minPrice)} (12 % de la valeur déclarée).`
              : 'Montant invalide.'}
          </FieldError>
        </div>
