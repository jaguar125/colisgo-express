...
import {
  ...
  MessageCircle, Send, Bell, Sun, Moon, Lock, Eye, EyeOff
} from 'lucide-react';

/* ---------------------------------------------------------------- */
/* Storage shim — same get/set/delete(key, shared) signature as the  */
/* Claude-artifacts `window.storage` API this file was written       */
/* against, backed by real `localStorage` for a normal deployment.   */
/* The `shared` flag is kept for API compatibility but has no effect */
/* here (there is no multi-user shared store outside the artifact    */
/* sandbox — all real "shared" data now lives in Supabase tables).   */
/* ---------------------------------------------------------------- */
window.storage = {
  async get(key) {
    try {
...
/* Sessions are kept in a localStorage-backed store (see `storage` below)  */
/* of localStorage, which artifacts aren't allowed to use.           */
/* ---------------------------------------------------------------- */
...
async function sbUpload(bucket, path, dataUrl, token) {
  ...
  if (!res.ok) throw new Error('Échec du téléversement de la photo');
...
.loading-screen{
  ...
  background:var(--ink); color:#9AA6D6; font-family:'Space Grotesk',sans-serif;
}

/* ---------- Landing page ---------- */
.landing{
  min-height:100vh; background:var(--paper); display:flex; justify-content:center;
}
.landing-shell{ width:100%; max-width:460px; min-height:100vh; background:var(--paper); }
...
function ChatWidget() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const stored = await window.storage.get('colisgo:chatIdentity', false);
  ...
  const startIdentification = async () => {
    ...
    setIdentity(info);
    try {
      await window.storage.set('colisgo:chatIdentity', JSON.stringify(info), false);
    } catch (e) {}
  };
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (window.location.hash.includes('access_token')) {
        ...
        try {
          const s = await window.storage.get('colisgo:session', false);
          if (s && s.value) {
            const parsed = JSON.parse(s.value);
            if (parsed.expires_at > Date.now()) {
              setSession(parsed);
              const row = await fetchProfileRow(parsed.user_id, parsed.email, parsed.access_token).catch(
                () => null
              );
              setProfile(
                row || {
  ...
  const persistSession = async (s) => {
    setSession(s);
    try {
      await window.storage.set('colisgo:session', JSON.stringify(s), false);
    } catch (e) {}
  };
  const clearSession = async () => {
    setSession(null);
    try {
      await window.storage.delete('colisgo:session', false);
    } catch (e) {}
  };
  ...
  const loadMatchings = async () => {
    try {
      ...
      notify('Impossible de charger les envois', 'error');
      return null;
    }
  };

  useEffect(() => {
    if (profile && session) loadMatchings();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profile ? profile.id : null, session ? session.access_token : null]);
    ...
    withBusy(async () => {
      ...
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/initier-paiement`, {
          ...
          body: JSON.stringify({ matching_id: shipment.id, operateur, telephone: profile.telephone }),
        });
        const data = await res.json();
        if (!res.ok || !data.payment_url) {
          throw new Error(data.error || 'Paiement indisponible — vérifiez la configuration PayDunya');
        }
        window.open(data.payment_url, '_blank');
        setPaymentFor(null);
        notify('Fenêtre de paiement ouverte — revenez ici une fois le paiement effectué', 'success');
      } catch (e) {
  ...
  if (loading) {
    return (
      <div className="cge">
        <style>{STYLES}</style>
        <div className="loading-screen">Chargement de ColisGo-Express…</div>
      </div>
    );
  }
        ...
        <div className="content">
          ...
          {tab === 'envois' && (
            <>
              ...
              {myShipmentsAsExpediteur.length === 0 && (
                <div className="empty-state">
                  <Package size={34} />
                  <div className="t">Aucun envoi en cours</div>
                  <div>Recherchez un trajet pour expédier votre premier colis.</div>
                </div>
              )}

              {myShipmentsAsExpediteur.map((s) => (
                ...
                <div className="ship-card" key={s.id}>
                  ...
                  {s.status === 'expedie' && (
                    <>
                      ...
                      <div className="helper" style={{ marginTop: 10 }}>
                        Demandez au destinataire le code à 4 chiffres qu'il a reçu, puis validez la remise.
                      </div>
                      <div className="code-entry">
                        <input
                          ...
                          onChange={(e) =>
                            setCodeInputs((c) => ({ ...c, [s.id]: onlyDigits(e.target.value, 4) }))
                          }
                        />
                      </div>
                    ...
                    <div className="ship-card" key={r.id}>
                      ...
                      <div className="admin-photos">
                        <div className="admin-photo-block">
                          {r.docImage ? <img src={r.docImage} alt="Pièce d'identité" /> : <div />}
                          <span>Pièce d'identité</span>
                        </div>
                        <div className="admin-photo-block">
                          {r.selfieImage ? <img src={r.selfieImage} alt="Selfie" /> : <div />}
                          <span>Selfie</span>
                        </div>
                      </div>
        ...
        <div className="field">
          ...
          {docImage ? (
            <div className="proof-preview">
              ...
              <button type="button" className="proof-retake" onClick={() => openPicker(docInputRef)}>
                Reprendre
              </button>
            </div>
          ) : (
            <button type="button" className="proof-upload" onClick={() => openPicker(docInputRef)}>
              <Camera size={16} />
...
function ShipmentFormSheet({ trip, onClose, onSubmit }) {
  ...
  const priceValid = proposedPrice !== '' && parseFloat(proposedPrice) > 0 && parseFloat(proposedPrice) <= 10000000;
      ...
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={'field' + (touched.proposedPrice && !priceValid ? ' has-error' : '')}>
          ...
          <input
            ...
            placeholder="15000"
          />
          <FieldError show={touched.proposedPrice && !priceValid}>Montant invalide.</FieldError>
        </div>
